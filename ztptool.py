...
def export_adom(adomname):
    ...
    std_objects = {
        ...
        "servicegrp": ["pm/config/adom/$(adom_name)/obj/firewall/service/group",
                    ...
                    ["uuid", "dynamic_mapping", "obj seq"]],
        "schedule": ["pm/config/adom/$(adom_name)/obj/firewall/schedule/recurring",
                     "pm/config/adom/" + adomname + "/obj/firewall/schedule/recurring/",
                     ["uuid"]],
    ...
    for sdwan_template in sdwan_template_list:
        ...
        get_and_add({"sdwan_member": ["pm/config/adom/$(adom_name)/wanprof/" + sdwan_template + "/system/virtual-wan-link/member",
                     "pm/config/adom/" + adomname + "/wanprof/" + sdwan_template + "/system/virtual-wan-link/member",
                     ["obj seq"]]}, "sdwan_member")

        get_and_add({"sdwan_hlth": ["pm/config/adom/$(adom_name)/wanprof/" + sdwan_template + "/system/virtual-wan-link/health-check",
                     "pm/config/adom/" + adomname + "/wanprof/" + sdwan_template + "/system/virtual-wan-link/health-check",
                     ["obj seq"]]}, "sdwan_hlth")


        get_and_add({"sdwan_service": ["pm/config/adom/$(adom_name)/wanprof/" + sdwan_template + "/system/virtual-wan-link/service",
...
def get_and_add(std_objects,objecturls):
    ...
    for index, config in enumerate(newdata):
        ...
        ignore_addr_obj = ["wildcard.dropbox.com", "wildcard.google.com", "SSLVPN_TUNNEL_ADDR1", "all", "gmail.com", "login.microsoft.com", "login.microsoftonline.com", "login.windows.net", "none", "FABRIC_DEVICE", "FIREWALL_AUTH_PORTAL_ADDRESS"]
        if objecturls is "addrobjs":
            if newdata[index]['name'] in ignore_addr_obj:
                newdata[index] = {}

        ignore_addrgrp_obj = ["G Suite", "Microsoft Office 365"]
        if objecturls is "addrobjs_grp":
            if newdata[index]['name'] in ignore_addrgrp_obj:
                newdata[index] = {}


        ignore_sdwan_hlth = ["Default_AWS", "Default_FortiGuard", "Default_Gmail", "Default_Google Search", "Default_Office_365"]
        if objecturls is "sdwan_hlth":
            if newdata[index]['name'] in ignore_sdwan_hlth:
                newdata[index] = {}

        ignore_schedule = ["always", "default-darrp-optimize", "none"]
        if objecturls is "schedule":
            if newdata[index]['name'] in ignore_schedule:
                newdata[index] = {}

        if objecturls is "sdwantemplates":
            sdwan_template_list.append(newdata[index]['name'])

        if objecturls is "polpkg":
            polpkg_list.append(newdata[index]['name'])

        if objecturls is "sdwan_service":
        ...
        ignore_service_obj = ["ALL", "ALL_TCP", "ALL_UDP", "ALL_ICMP", "ALL_ICMP6", "GRE", "GTP", "AH", "ESP", "AOL", "BGP", "DHCP", "DNS", "FINGER", "FTP", "FTP_GET", "FTP_PUT", "GOPHER", "H323", "HTTP", "HTTPS", "IKE", "IMAP", "IMAPS", "Internet-Locator-Service", "IRC", "L2TP", "LDAP", "NetMeeting", "NFS", "NNTP", "NTP", "OSPF", "PC-Anywhere", "PING", "TIMESTAMP", "INFO_REQUEST", "INFO_ADDRESS", "ONC-RPC", "DCE-RPC", "POP3", "POP3S", "PPTP", "QUAKE", "RAUDIO", "REXEC", "RIP", "RLOGIN", "RSH", "SCCP", "SIP", "SIP-MSNmessenger", "SAMBA", "SMTP", "SMTPS", "SNMP", "SSH", "SYSLOG", "TALK", "TELNET", "TFTP", "MGCP", "UUCP", "VDOLIVE", "WAIS", "WINFRAME", "X-WINDOWS", "PING6", "MS-SQL", "MYSQL", "RDP", "VNC", "DHCP6", "SQUID", "SOCKS", "WINS", "RADIUS", "RADIUS-OLD", "CVSPSERVER", "AFS3", "TRACEROUTE", "RTSP", "MMS", "KERBEROS", "LDAP_UDP", "SMB", "NONE", "webproxy"]
        if objecturls is "service":
            if newdata[index]['name'] in ignore_service_obj:
                newdata[index] = {}

        ignore_servicegrp_obj = ["Email Access", "Web Access", "Windows AD", "Exchange Server"]
        if objecturls is "servicegrp":
...
def btn_checkxlsx(filename, fmghost, fmguser, fmgpasswd, fmgadom):
    ...
    if proceed == True:
        ...
        required_headings = ["Device_Name", "Platform", "Device_SN", "CLI_Template", "Post_CLI_Template", "Policy_Package",
                             "SDWAN_Template"]
        ...
        for devicedata in alldevices:
            ...
            if add_dev_status == True:
                ...
                if devicedata['CLI_Template'] is None:
                    ...
                    status_clitempgrp = assign_cli_template(fmg_adom, devicedata['CLI_Template'], devicedata['Device_Name'])
            ...
            if qi_status == True:
                ...
                if devicedata['SDWAN_Template'] is None:
                    return_html += "Assign SD-WAN Template {not defined} <span class=\"glyphicon glyphicon-info-sign\" style=\"color:orange\"></span><br>\n"
                else:
                ...
                if devicedata['Post_CLI_Template'] is None:
                    ...
                    status_clitempgrp = assign_cli_template(fmg_adom, devicedata['Post_CLI_Template'], devicedata['Device_Name'])
                    if status_clitempgrp == "OK":
...
eel.start('ztptool.html', size=(790, 850), disable_cache=True)  # Start (this blocks and enters loop)
